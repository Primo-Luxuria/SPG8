...
class Textbook(models.Model):
    ...
    def get_feedback(self):
        ...
        return Feedback.objects.filter(
            Q(question__textbook__isbn=self.isbn) |
            Q(test__textbook__isbn=self.isbn) |
            Q(test__course__textbook__isbn=self.isbn)
        ).distinct()
...
class Course(models.Model):
    ...
    def get_publisher_tests(self):
        if self.textbook:
            return Test.objects.filter(
                textbook=self.textbook,
                textbook__publisher__userprofile__role='publisher'
            )
